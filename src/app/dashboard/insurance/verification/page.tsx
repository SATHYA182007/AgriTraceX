      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] font-black uppercase tracking-[0.4em] text-rose-500">Insurance Protocol</p>
          </div>
          <h1 className="text-5xl font-manrope font-extrabold tracking-tighter text-foreground">
            Verification <span className="text-rose-500">Queue</span>
          </h1>
        </div>
      </div>
              ...
              {VERIFICATIONS.map((v) => (
                <tr key={v.id} className="hover:bg-surface-soft/30 transition-colors">
                  ...
                  <td className="p-6 flex justify-end gap-3">
                    <Button variant="ghost" size="icon" onClick={() => setSelected(v)} className="h-10 w-10 text-rose-500 bg-rose-50 hover:bg-rose-100 rounded-xl" title="View Details">
                      <Eye size={16} />
                    </Button>
                    ...
                    <Button variant="ghost" size="icon" className="h-10 w-10 text-indigo-500 bg-indigo-50 hover:bg-indigo-100 rounded-xl" title="Request Inspection">
                      <ShieldAlert size={16} />
                    </Button>
                  </td>
                </tr>
            ...
            <div className="bg-white rounded-[32px] w-full max-w-4xl p-10 relative overflow-hidden" onClick={e => e.stopPropagation()}>
               ...
               <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                     <div>
                        <h4 className="text-sm font-black uppercase tracking-widest text-text-soft mb-2">Metrics & Evidence</h4>
                        <div className="grid grid-cols-2 gap-4">
                           <div className="p-4 rounded-xl bg-surface-soft"><span className="text-[10px] uppercase font-bold text-text-soft">Analyst Cert</span><p className="font-jetbrains font-bold text-emerald-500">Verified</p></div>
                           <div className="p-4 rounded-xl bg-surface-soft"><span className="text-[10px] uppercase font-bold text-text-soft">Damage %</span><p className="font-jetbrains font-bold text-rose-500">68%</p></div>
                        </div>
                     </div>
                     <div>
                        ...
                        <p className="text-sm font-medium text-foreground p-4 rounded-xl border border-border bg-white shadow-sm">
                           Severe heat stress observed. Leaves are wilting. Uploaded 4 drone images as evidence. Requesting immediate inspection.
                        </p>
                     </div>
                     <div className="flex justify-end gap-4 mt-6">
                        <Button className="bg-emerald-500 text-white rounded-xl font-bold">Approve</Button>
                        <Button className="bg-indigo-500 text-white rounded-xl font-bold">Request Inspection</Button>
                     </div>
                  </div>
               </div>
            </div>
